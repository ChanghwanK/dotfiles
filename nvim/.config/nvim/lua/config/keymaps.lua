...
-- [비활성화] Neo-tree 단축키 — snacks.picker.explorer로 대체. 롤백: 아래 주석 해제 + snacks 줄 제거
...
mapKey('<S-h>', '<cmd>BufferLineCyclePrev<CR>', 'n', { desc = "이전 버퍼로 이동" })
mapKey('<S-l>', '<cmd>BufferLineCycleNext<CR>', 'n', { desc = "다음 버퍼로 이동" })
mapKey('<C-w>', function() pcall(Snacks.bufdelete) end, 'n', { desc = "현재 버퍼 닫기 (Ctrl+w)" })
...
mapKey('<leader>bl', '<cmd>BufferLineCloseLeft<CR>', 'n', { desc = "왼쪽 버퍼 모두 닫기" })
mapKey('<leader>br', '<cmd>BufferLineCloseRight<CR>', 'n', { desc = "오른쪽 버퍼 모두 닫기" })

-- blamer.nvim (인라인 Git Blame, VSCode GitLens 스타일)
mapKey('<leader>gb', ':BlamerToggle<CR>', 'n', { desc = "Git Blame 인라인 토글" })

-- Lazygit (Git UI)
mapKey('<leader>gg', function() Snacks.lazygit() end, 'n', { desc = "Lazygit 열기" })
mapKey('<leader>gf', function() Snacks.lazygit.log_file() end, 'n', { desc = "Lazygit: 현재 파일 로그" })
