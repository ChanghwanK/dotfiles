...
description: |
  ...
  상태가 '해야할 것'이면 '진행 중'으로 전환한다(이미 진행 중이면 무변경, 멱등).
...
- **상태 전이는 단방향·멱등**: '해야할 것' -> '진행 중' 전환만 한다. 이미
...
### Step 2: 상태 전환 ('해야할 것' -> '진행 중', 조건부)

Step 1에서 얻은 `status` 값으로 분기한다:

| status | 동작 |
|--------|------|
| `해야할 것` | `update-status`로 '진행 중' 전환 |
