...
### 1. 우선순위 검증: 안정성 > 비용 > 가용성 수준

세 요소는 같은 층위의 트레이드오프 축이 아니라 역할이 다르다 (목적 / 제약 / 조절 변수).
주제({topic})와 목표({goal_summary})를 분석:
- **안정성 영향 (목적)**: 실수하기 어려워지는가? 실패해도 빠르고 예측 가능하게 복구되는가? (가드레일, 자동화, 셀프서비스, 관측성, correctness 가드가 전부 여기 속한다)
- **비용 영향 (제약)**: 비용이 증가하는가? 같은 안정성 효과를 더 싸게 낼 방법은 없는가?
- **가용성 수준 영향 (조절 변수)**: 9의 개수·이중화 범위를 올리는가? 올린다면 그 비용에 비례하는가? 안정성과 혼동하지 않는다. 과잉투자 금지
- **플랫폼(수단) 확인**: 병목 해소·self-service는 그 자체가 목표가 아니라 안정성을 인지부하 없이 달성하는 수단이다. "개발자가 편해진다"만으로는 착수 근거가 되지 않으며 안정성 기여로 설명되어야 한다
