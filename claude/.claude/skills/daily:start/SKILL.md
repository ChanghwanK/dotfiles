...
**[진행]/[검토] 분류 (후보 풀 내에서 각 항목에 태그 부여):**
- `[진행]`: 어제 Obsidian `[ ]` 미완료이면서, 어제 transcript에서 관련 분석/설계 작업이 있었던 항목. 즉, 분석은 완료되었고 실행만 남은 상태.
- `[검토]`: 그 외 모든 항목 (분석/설계가 아직 필요한 것).
- 판단이 애매하면 `[검토]`으로 분류한다.

**우선순위 가중치 (후보 풀 내에서만 적용):**
- 어제 "내일 해야할 것"에 명시됨: +4점 (사용자 직접 의도, 최고 가중치)
- `[진행]` 타입 (분석 완료, 실행만 남음): +3점
- Priority = P1: +3점 / P2: +1점
- Notion status = 진행 중: +2점 (모멘텀 유지)
- 오늘이 due_end 당일 또는 범위 내: +2점 (마감 임박)
- 어제 Obsidian `[ ]` 미완료이며 carry-over: +1점

**Top 3 정렬:** `[진행]` 태그 항목을 상단에 배치한다 (가중치 합산 후, 동점이면 [진행] 우선).

**remaining_tasks 정의:** 후보 풀에서 Top 3로 선정되지 않은 Notion daily tasks 전체.
이 목록을 Step 4(콘솔 `📋 오늘 할 일 전체`) 및 Step 5(Obsidian `## Todos`)에서 사용한다.
...
`📋 오늘 할 일 전체` 섹션에 Top 3와 remaining_tasks를 구분하여 출력한다.
...
- `## Top 3 오늘의 목표`: Step 3에서 선정한 Top 3로 채움. 형식: `- [ ] [진행|검토] 작업명` + 탭 들여쓰기로 `\t- 완료 조건: {조건}` + `\t- 이유: {이유}`. `[진행]` 항목을 상단에 배치.
...
- `## Todos`: Top 3 항목 + remaining_tasks 전체 + `tomorrow_tasks` 중 위 목록에 없는 항목을 `- [ ]` 형식으로 채움. Top 3 항목에는 `[Top 3]` 접두사를 붙여 구분. **`weekly_project` Task는 포함하지 않는다.**
...
| Todos | ✅ | **보존 + tomorrow_tasks append** | Top 3 + remaining_tasks + tomorrow_tasks 미포함분을 `- [ ]` 형식으로 채움 |
