...
**Agent 2개를 동시에 launch**하여 데이터 수집을 독립 서브프로세스에 위임한다.
...
---
weekly_tasks:
  - name: "작업명", priority: P1, status: 진행중, due: 2026-03-10~2026-03-14
  - ...
...
두 Agent를 **단일 메시지에서 동시에 launch**한다 (독립적이므로 병렬 실행 가능):
...
두 Agent 결과를 모두 수신한 후 Step 2로 진행한다.
...
4. carry-over = 어제 미완료 중 오늘 Notion Todos에 없는 항목
...
- `## Top 3 오늘의 목표`: Step 3에서 선정한 Top 3로 채움. 부모 `- [ ] 작업명`에 자식 `\t- [ ] 이유`를 탭 들여쓰기로 중첩.
- `## 어제 리뷰 (YYYY-MM-DD)`: 실제 어제 날짜 기입. 완료/진행 중/carry-over 섹션으로 구성.
- `## 오늘 일정`: Agent B의 `calendar_events`로 채움. 없으면 `- (일정 없음)`.
- `## Todos`: Top 3 항목 + `tomorrow_tasks` 중 Top 3에 포함되지 않은 항목을 `- [ ]` 형식으로 채움.
...
```
AskUserQuestion:
  question: "오늘 Notion Todo's를 Top 3 항목으로 업데이트할까요?"
  options:
    - label: "Yes"
      description: "Top 3 항목으로 오늘 Notion Todo's를 업데이트합니다."
    - label: "No"
      description: "업데이트하지 않고 넘어갑니다."
```

- "Yes" 선택 시 `update-todos` 커맨드로 오늘 페이지에 반영. **Top 3 + tomorrow_tasks 미포함분을 모두 포함**하여 Obsidian Todos와 동기화.
- "No" 선택 시 추가 작업 없이 종료
