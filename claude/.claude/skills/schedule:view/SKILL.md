...
name: schedule:view
description: |
  Notion Task/Goal/Daily DB 기반 이번 주 일정 현황 스킬.
...
스크립트 실행 후 JSON 응답의 `success` 필드를 확인한다.

실패 시:
- `NOTION_TOKEN not set` → `~/.secrets.zsh`에서 `NOTION_TOKEN` 환경변수 확인
- `error: database not found` → DB ID 확인 (`notion-schedule.py` 내 상수값)
- 빈 결과 → Notion Task DB 필터 조건 및 이번 주 날짜 범위 확인
