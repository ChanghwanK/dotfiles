...
### 2단계: claude-mem timeline 교차 검증 (best-effort)
...
**케이스 A — 상태 불일치 감지 시** (Notion "시작 전" + 세션 작업 있음):
```
🎩 중간 점검 — 2026-MM-DD (요일)
...
[상태 동기화 필요]
- '{Task명}': Notion은 '시작 전'이지만 오늘 세션에서 작업이 감지됐습니다.
...
목적은 추상어 "안심"을 **"5개 중 몇 개 충족"이라는 숫자**로 바꿔 **'동작함'과 '끝남'을 구분**하는 것이다.
...
먼저 아래 2개를 interactive하게 묻는다. "했다"는 답은 받지 않고 **근거 1개**(명령어 출력·메트릭·로그·링크·절차)를 요구한다. 근거 없으면 보완 요청 후 재답변.

1. **동작 확인** — 어떻게 확인했습니까? (명령어 출력 / 메트릭 / 로그 / 스크린샷 중 1개)
2. **롤백 경로** — 터지면 어떻게 되돌립니까? (명령어 또는 절차 한 줄)

두 개 모두 근거가 있으면 → 3단계로 진행. 하나라도 없으면 → 보완 요청 후 재답변.

### 3단계: 보완 권고 (선택 3개) + 판정

핵심 2개 통과 후, 아래 3개를 **한꺼번에 권고 형태로** 보여준다 (답변 요구 아님).

3. **실패 케이스** — 경계/실패 케이스를 봤습니까? (권한없음·중복·타임아웃 등)
4. **나쁜 소식 선공유** — 지연·리스크를 마감 전에 알렸습니까? (없으면 "리스크 없음")
5. **상대 추가 작업** — 맡긴 사람이 추가로 할 일이 있습니까?

판정:
- 핵심 2개 ✓ + 보완 2개 이상 ✓ → 안심 (4~5점)
- 핵심 2개 ✓ + 보완 1개 이하 ✓ → 경계 (3점)
- 핵심 2개 중 하나라도 ✗ → 보류
...
- **보류** 판정 시 클로징 시퀀스(4단계)를 실행하지 않는다. 5체크 재시도만 안내한다.
- Task 삭제는 이 모드에서 하지 않는다 — 완료 처리는 상태를 "완료"로 변경하는 것이다.
- 5체크 외 항목은 묻지 않는다. 잔소리꾼이 되지 않는 것이 목적이다.
