...
> **하드 가드레일 — 호출 컨텍스트 면죄 금지**: 호출자(메인 Claude 등)가 작업 결과 요약을 미리 채워줬더라도, 그것을 4체크 답변으로 간주하지 않는다. 4체크는 **반드시 사용자에게 직접 제시하고 사용자의 실시간 답변을 기다려야** 한다. 호출 프롬프트의 맥락을 아무리 상세히 읽었어도 이 단계를 건너뛰지 않는다.

아래 4개를 **한꺼번에 권고 형태로** 보여준다. 답변을 강요하지 않고, **이 질문을 받은 사용자가 직접 응답한 내용**만 충족으로 처리한다(호출 시 전달된 사전 맥락은 충족 근거로 쓰지 않는다).
