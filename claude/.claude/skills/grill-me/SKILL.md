...
- **SOCRAAI 원칙 연결**: 모든 branch에서 안정성 > 비용 > 가용성 수준 트레이드오프 체크
...
- 관련 컴포넌트: {영향받는 시스템}
- SOCRAAI 원칙: {안정성/비용/가용성 수준 중 관련 축}
```
...
| 원칙 | 체크 포인트 | 예시 질문 |
|------|------------|----------|
| 안정성 > 비용 > 가용성 수준 | 줄인 것이 가용성 수준인가, 안정성까지 깎았는가? | "이 결정에서 낮춘 것이 9의 개수·이중화 범위인가요, 아니면 가드레일·복구성인가요?" |
