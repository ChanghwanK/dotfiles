...
| **ROI** | 아래 "ROI 자동 분류 (질문 없음)" 참조. 사용자에게 묻지 않음 | 애매하면 미설정 |
...
**GTD Inbox 원칙("캡처 ≠ 의사결정")을 지키기 위해 이 분류는 사용자에게 묻지 않는다** — Priority/Due Date와 달리 확인 게이트가 없다. Claude가 제목·description만으로 조용히 추정해 확신이 서면 즉시 반영하고, 애매하면 미설정으로 남겨 `/alfred groom`이 나중에 처리하게 한다.

1. [work-definition-framework.md](~/workspace/riiid/kubernetes/devops-wiki/01-decisions/work-definition-framework.md)의 6유형 + 판단 순서(10문항)로 분류해 L1/L2/L3/보류 레벨을 추정한다.
2. 같은 문서의 "Quick-Win 상향 규칙"을 적용한다 — 파일 1개 이하의 단일 값/설정/문구 교체, 오타·링크·주석 수정, 명령어 한 번으로 끝나는 수정형 작업이면 L3→Medium, L2→High로 한 단계 올린다.
3. "Notion Task ROI 매핑" 표로 레벨을 High/Medium/Low로 변환한다.
4. **확신 기준**: 제목·description만으로 유형이 뚜렷하면(예: "장애", "OOM", "오타 수정", "이미지 태그 1건 변경" 등 신호가 명확) `--roi`를 즉시 반영한다. 유형이 여러 개에 걸치거나 임팩트를 가늠하기 어려우면(예: "~ 검토해보기", "언젠가 ~") `--roi`를 생략한다(미설정, groom 재검토 대상).
5. 단순 한 줄 메모(P3/P4 + description 없음)라도 이 분류는 동일하게 적용한다 — 본문 템플릿 여부와 무관하다.
...
- **Non-Goals는 오버엔지니어링 방지 관점**에서 작성한다. "이번엔 가시성만, 자동화는 범위 외" 같이
  범위를 좁히는 경계를 1~2개 제시한다.
...
- **ROI 분류 (질문 없음)**: "ROI 자동 분류" 섹션을 적용해 확신이 서면 값을 정하고, 애매하면 미설정으로 둔다. 이 단계는 사용자에게 묻지 않는다.
...
> 본문 헤딩은 `## 문제 정의` / `## 해결 이유` / `## Summary` / `## 기대 효과` / `## Non-Goals` 5개를 고정으로 사용한다.
...
```
📥 캡처 완료: [P3] Task 이름
📥 캡처 완료: [P2][ROI Medium] Task 이름 (~2026-03-20)
