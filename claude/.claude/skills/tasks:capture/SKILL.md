...
**본격 Task**에만 4-필드 본문 템플릿을 페이지 본문에 렌더링한다.
...
> 자동 추천 P2는 키워드만으로 붙어 정보량이 부족할 수 있다. 이 경우 4-필드를 추정으로 채우지 말고
> `(TBD)`로 표시한 뒤 **Step 2-A 단일 확인**에서 사용자가 거른다 — 확인 단계가 게이트 역할을 한다.
...
## Why
{왜 하는지 — 해결하려는 문제·동기. bullet 또는 문장}
...
- **Claude가 대화 맥락에서 자동 합성한다.** 추가 질문 없이 초안을 만든다.
- **Summary는 반드시 3줄 이상.** 정보가 부족하면 추정으로 채우지 말고
  `- (TBD) {확인 필요한 항목}` 형태로 명시하여 빈 칸이 드러나게 한다.
...
- **본격 판정**: "본문 템플릿 (본격 Task)" 섹션의 판정 기준을 적용한다.
  본격이면 4-필드 초안을 "합성 가이드"에 따라 미리 합성한다.

### Step 2 — 확인 및 질문

**본격 Task 여부에 따라 분기한다.**

#### Step 2-A — 본격 Task (4-필드 본문 적용)

합성한 4-필드 초안 + 확정될 속성(Priority/Due)을 **한 번에** 보여주고 단일 확인한다.
...
# 본격 Task — 4-필드 본문 템플릿을 --body 인라인 Markdown으로 전달
...
> 본문 헤딩은 `## Summary` / `## Why` / `## 기대 효과` / `## Non-Goals` 4개를 고정으로 사용한다.
