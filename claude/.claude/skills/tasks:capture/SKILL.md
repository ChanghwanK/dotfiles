...
## 01. 문제 정의
- {어떤 경계·장치·절차가 제 역할을 못 하는가 (현상 금지, 합성 가이드 "현상을 빼고 문제를 쓴다" 참조)}
- *As-Is:*
  - {지금 시스템이 허용하는 실패 경로}
- *To-Be:*
  - {같은 조건에서 시스템이 보여야 할 동작}

## 02. 해결 이유
- {왜 지금 해결해야 하는지: 관측된 현상(시각, 횟수, 지속 시간, 재발 이력)은 여기에 근거로 쓴다}
- {방치 시 발생하는 영향}
- *검토한 대안:*
  - {대안}: {기각 근거}
...
- **`01. 문제 정의`는 현상을 빼고 문제를 쓴다.** 현상은 "무엇이 일어났다"(관측된 사건)이고, 문제는
  "무엇이 작동하지 않아서 그 사건이 가능했는가"(구조의 결함)다. 알럿·RCA에서 출발한 Task는 관측
  타임라인을 그대로 01에 옮기기 쉬우므로, 초안을 쓴 뒤 아래 판별을 문장마다 적용한다.
  - *판별:*
    - 날짜·시각·횟수·지속 시간·노드/파드 이름을 지워도 문장이 성립하는가? 무너지면 현상이다
    - 다음에 같은 일이 또 나도 문장이 그대로 참인가? 이번 건만 설명하면 현상이다
    - 주어가 사건("노드가 NotReady가 됐다")인가, 경계·장치·절차("노드 경계가 파드 장애를 가두지 못한다")인가
    - 문제 유형에 표준 용어로 이름을 붙일 수 있는가(장애 격리 실패, retry storm, 자동 복구 부재 등)? 못 붙이면 아직 현상 수준이다
  - *현상의 자리:*
    - 관측값은 버리지 않고 `02. 해결 이유`의 근거(재발 이력, 지속 시간, 영향 범위)로 옮긴다. 01과 02에 같은 사실을 두 번 쓰지 않는다
  - *시제:*
    - 01은 현재형으로 쓴다("~하지 못한다"). 과거형("~했다")이 나오면 관측을 옮겨 적고 있다는 신호다
  - *범위 주장 대조:*
    - "하나", "매번", "일상", "방치됐다" 같은 범위·빈도·완료 표현은 증거와 대조한다(여러 건이면 단수로 쓰지 않고, 진행 중인 일은 완료형으로 쓰지 않는다)
  - *예시(2026-09-26 prod 노드 NotReady):*
    - 나쁜 예(현상): "09-24 04:40 노드가 NotReady가 된 뒤 55시간 복구되지 않았고, 09-03에도 노드 3대가 7시간 NotReady였다"
    - 좋은 예(문제): "브로커 정기 유지보수처럼 예고된 인프라 이벤트에서 워커 하나가 비정상이 되면, 컨테이너 경계(memory limit)와 노드 경계(kubelet 보호)가 그 장애를 가두지 못해 노드 전체로 번지고, 번진 노드를 자동으로 교체하는 경로가 없어 사람이 개입할 때까지 복구되지 않는다"
    - 나쁜 예의 날짜·시간·대수는 02의 근거로 옮긴다
...
A5(현상 제거)는 RCA 판정이 아니라 01 문장 작성 검사이므로 정본 유무와 관계없이 항상 적용한다.
...
| A5 | 01 초안에서 날짜·횟수·지속 시간·리소스 이름을 지우면 문장이 무너지는가? | 무너지면 현상이다. 관측값은 02로 옮기고, 그 사건을 가능하게 한 경계·장치·절차의 결함으로 다시 쓴다(합성 가이드 "현상을 빼고 문제를 쓴다") |
