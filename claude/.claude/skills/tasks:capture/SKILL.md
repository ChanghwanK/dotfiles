...
| **Description** | "why:", "because", "이유:", "목표:", `:` 이후 부연 설명 문장 등 컨텍스트 정보 | 없음 (선택) |
...
- **Description 추출 규칙**: 입력에 이름과 함께 "why:", "because", "이유:", "목표:", `,` 이후 부연 설명 등 맥락 정보가 있으면 description으로 분리. 이름은 핵심 동작/태스크만 남긴다.

  예시:
  - `slack app 통합, devops/ArgoCD 등 다양하게 사용 중인 app을 통일하는 것이 목표 why? 접근 권한이 없음 유지보수 안됨`
    → 이름: `Slack App 통합 및 중앙화`
    → description: `devops, Argocd 등 다양하게 사용 중인 app을 통일하는 것이 목표 why? 접근 권한이 없음 유지보수 안됨`
...
# Description 포함 (맥락/이유가 있는 경우)
python3 /Users/changhwan/.claude/skills/tasks:manage/scripts/notion-task.py \
  create-task --name "Task 이름" --priority "P3 - Could Have" --category "WORK" \
  --description "배경 및 이유 설명"
