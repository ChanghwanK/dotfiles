...
우리 팀 원칙: **안정성 > 비용 > 가용성 수준**
- dev/stg: 가용성 수준은 최소로 (Single AZ, 최소 사양). 단 **안정성(가드레일, 복구성)은 prod와 동일하게 적용한다** (dev에서 통한 실수는 prod에서도 통하므로 실수 방지 규칙을 환경별로 다르게 두지 않는다)
- prod: 가용성 수준은 비용에 비례하는 만큼만 (과잉 프로비저닝 지양). 낮추는 대상은 9의 개수이지 가드레일이 아니다
