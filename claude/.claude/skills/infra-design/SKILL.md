...
### Step 0: 모호성 인터뷰 (모호한 요청일 때만)

요청만으로 아래 세 가지 중 하나라도 확정할 수 없으면, 조사를 시작하기 전에 `AskUserQuestion`으로 선택지를 제시해 사용자가 선택하게 한다. 요청이 이미 명확하면 이 Step은 건너뛰고 바로 Step 1로 간다.

- **무엇을 (대상·범위)**: 설계 대상이 무엇인지, 어디까지 포함하는지 (예: 클러스터 신규 구축 vs 기존 확장 vs 대안 비교만)
- **왜 (동인)**: 신규 기능 요구 / 비용 절감 / 성능·확장성 / 보안·컴플라이언스 / 운영 부담 해소
- **어떻게 (제약·환경)**: 대상 환경(prod/stg/dev/IDC), 규모 수준, Managed 우선 여부

규칙:
- `AskUserQuestion` 1회 호출에 최대 4문항, 각 문항 2~4개 선택지로 묶는다 (여러 번 왕복하지 않는다)
- 선택지는 범용 나열이 아니라 요청 문맥에서 파생한 구체안으로 만든다 (사용자는 Other로 직접 입력도 가능)
- 선택 결과를 Step 1의 요구사항 정리에 반영하고, 출력의 "요구사항 정리" 섹션에 인터뷰로 확정된 항목임을 표기한다

### Step 1: 설계 요구사항 구체화

요청(+ Step 0 인터뷰 결과)에서 다음을 확정한다. 여기서 새로 발견된 불명확 항목은 추가 질문으로 해소한다:
