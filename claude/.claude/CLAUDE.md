...
ExitPlanMode 완료 후 플랜 `.md` 파일이 저장되면, 즉시 동일 내용을 HTML로 렌더링하여 같은 경로에 `.html` 파일로 함께 저장한다.

**목적:** 브라우저에서 플랜을 가독성 높게 확인하기 위함.

**규칙:**
- 파일 경로: `.md`와 동일 디렉터리, 확장자만 `.html`로 변경
  - 예: `~/.claude/plans/upgrade-mango.md` → `~/.claude/plans/upgrade-mango.html`
- HTML 저장 후 `open "file:///absolute/path/to/plan.html"` 명령어를 Bash로 실행하여 브라우저를 자동으로 연다
