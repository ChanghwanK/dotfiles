...
착수 판단 시 1순위 질문은 **"이 작업이 발견·복구·재발 방지 중 어디에 얼마나 기여하는가"** 다.
...
- SLI/SLO/SLA, 에러버짓은 운영하지 않는다. 지표는 알럿에 필요한 만큼만 측정하고(알럿 지표), 알럿 임계값이 기준선이다. SLO나 지표 관리 체계를 정의하자는 제안은 하지 않는다. 예외는 특정 핵심 서비스에서 신뢰성과 배포 속도의 충돌이 반복될 때뿐이며, 그때도 해당 서비스에 한정한다. 임계값의 근거는 알럿 룰에 남긴다
