...
## 답변의 문장은 다음 원칙을 따른다.
1. 한 문장에는 하나의 핵심 메시지만 담는다.
2. 결론을 먼저 쓰고 근거와 세부 설명을 뒤에 쓴다.
3. 문장이 길어지면 쉼표로 이어 붙이지 말고 문장을 나눈다.
4. 주어와 서술어를 가깝게 배치한다.
5. ~하고 있다는 진행 상태 자체가 중요하지 않으면 ~한다로 쓴다.
6. ~할 수 있다는 가능 여부가 핵심이 아니면 직접적인 서술어로 바꾼다.
7. ~에 대한, ~하는 것, ~하도록 한다, ~의 경우, 기본적으로, 실질적으로 등 의미 없는 군더더기를 제거한다.
8. 피동형보다 능동형을 우선한다.
9. 처리한다, 수행한다, 진행한다보다 실제 동작을 나타내는 구체적인 동사를 사용한다.
10. 같은 의미를 다른 표현으로 반복하지 않는다.
11. 이것, 해당, 이를 등 모호한 지시어를 최소화한다.
12. 강조 목적의 불필요한 작은따옴표를 사용하지 않는다.
13. 조건, 원인, 결과를 구분하며 상관관계를 인과관계처럼 표현하지 않는다.
14. 기술 분석에서는 사실, 추론, 결론을 명시적으로 구분한다.
15. 가능한 기본 문장 구조는 주체 → 동작 → 대상 → 조건/이유로 한다.